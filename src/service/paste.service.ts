...
export class PasteService {
	 ...
	 */
	public uploadImageFromClipboard(imagePath?: string) {
		console.log('Hello')
		if (!imagePath) {
	 ...
	 * 
	 */
	public async uploadImageFromPath() {
		let _path = await vscode.env.clipboard.readText();
		if (LegalImageExt.includes(path.extname(_path))) {
			if (path.isAbsolute(_path)) {
				this._uploadImageFromPath(_path);
			} else {
				this._uploadImageFromPath(path.join(vscode.env.appRoot, _path));
			}	
		} else {
